...
convSigCt <- function(phy, traits, focaltaxa, groups = NULL, nsim=1e3, ...)	{
	data <- calcConvCt(phy, traits, focaltaxa, groups, ...)
	
	# run simulations
	phylMat <- vcv.phylo(phy)
	phylMat2 <- phyl.vcv(traits, phylMat, 0)
	simDat <- sim.char(phy, phylMat2$R, nsim, model="BM", root=0)
	simOut <- apply(simDat, 3, calcConvCt, phy=phy, focaltaxa=focaltaxa, groups = groups, VERBOSE=FALSE, ...)	## not 100% sure the ellipses are working here? Should null have the same limitation to Dmax?
	
	
	simAvg <- sapply(simOut,"[[",1)
	pvals <- sapply(1:4, function(x) length(which(simAvg[x,] >= data[["mean"]][x]))) / nsim
	out <- cbind(data[["mean"]], pvals)
	
	to_return<-list("pvals" = out, "meas.Cmat" = data$Cmat, "meas.path" = data$meas.path, "sim.avg" = simAvg, "sim.path" = do.call(c,sapply(simOut,"[",3)))
	
	if(!is.null(groups)){
	grpAvg <- lapply(simOut,"[[",4)
	grp.pvals <- matrix(sapply(1:prod(dim(data$grp.mean)),function(x) length(which(lapply(grpAvg,"[[",x) >= data$grp.mean[[x]]))/nsim),4,ncol(data$grp.mean))
	...
	print(list(round(to_return$grp.mean,3), round(to_return$grp.pvals,3)))
	} else {print(round(to_return$pvals,3))}
